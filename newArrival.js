...
asc.onclick = async () => {
    try{
        let response = await fetch(`http://localhost:3000/New_Arrival?_page=1&_limit=15_sort=items_price&_order=desc`);
...
async function collectcard(){
    ...
    try{
        let response = await fetch("http://localhost:3000/New_Arrival?_page=1&_limit=15");
        let data = await response.json();
        console.log(data)
        appendData(data)
    }
